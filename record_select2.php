...
if($_SESSION['username'] != null)
{
	...
	$sql = "SELECT * FROM record_table where username = '$username' ";
        $result = mysql_query($sql);
        $row = mysql_fetch_row($result);
	
        echo "<form style='width:980px;margin: auto;' name=\"form\" method=\"post\" action=\"record_select_finish.php\">";
		echo "<div style='margin-left:390px;margin-top:100px;	font-size:30px;letter-spacing: normal;font-weight:900;color: #1F7CFF;'>唱片資料查詢 </div>";
        ...
        echo "<p style='position:absolute;margin-left:240px;margin-top:15px;'>作曲者</p>
        		<input style='margin-left:290px;margin-top:10px; width:380px; height:30px;border: 2px solid rgba(204, 218, 224, 1);font: normal normal normal 16px/1.4em overlock, sans-serif;' type=\"text\" name=\"composer\" value=\"$row[3]\" /> <br>";
	echo "<p style='position:absolute;margin-left:240px;margin-top:15px;'>作詞人</p>
			<input style='margin-left:290px;margin-top:10px; width:380px; height:30px;border: 2px solid rgba(204, 218, 224, 1);font: normal normal normal 16px/1.4em overlock, sans-serif;' type=\"text\" name=\"lyricist\"  value=\"$row[4]\" />  <br>";
