...
if($ID != null )
{
        $sql = "select * from h_employee_table where ID='$ID'";
        $data=mysql_query("select * from h_employee_table where ID='$ID'");
        if(mysql_query($sql)==false)
        {
                echo "<script>alert('ERROR!');window.location='h_employee_select.php';</script>";
...
else
{
        if($name != null )
        {
        $sql = "select * from h_employee_table where name='$name'";
        $data=mysql_query("select * from h_employee_table where name='$name'");
                if(mysql_query($sql)==false)
                {
                echo "<script>alert('ERROR!');window.location='h_employee_select.php';</script>";
        ...
        else
        {
                if($phone != null )
                {
                $sql = "select * from h_employee_table where phone='$phone'";
                $data=mysql_query("select * from h_employee_table where phone='$phone'");
                        if(mysql_query($sql)==false)
                        {
                        echo "<script>alert('ERROR!');window.location='h_employee_select.php';</script>";
                ...
                else
                {
                       if($hourrate != null )
                        {
                        $sql = "select * from h_employee_table where hourly_rate='$hourrate'";
                        $data=mysql_query("select * from h_employee_table where hourly_rate='$hourrate'");
                                if(mysql_query($sql)==false)
                                {
                                echo "<script>alert('ERROR!');window.location='h_employee_select.php';</script>";
                        ...
                        {
                        echo "<script>alert('請輸入搜尋項目!');window.location='h_employee_select.php';</script>";
                        }                       
                }
        }
}
